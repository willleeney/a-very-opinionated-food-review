...
const TEASER_COMMENTS = [
  'A solid lunch spot with great options',
  'Worth the walk — really enjoyed it',
  'Decent portions and quick service',
  'One of the better places nearby',
  'Good value for the area',
  'Surprisingly good, will go back',
  'Perfect for a quick team lunch',
  'Great flavours, friendly staff',
]
...
export function LandingPage() {
  ...
  const allReviews = useMemo((): ReviewFlat[] => {
    ...
    for (const r of restaurants) {
      for (const rev of r.reviews) {
        if (rev.rating === null) continue
        flat.push({
          id: rev.id,
          rating: rev.rating,
          comment: TEASER_COMMENTS[commentIndex % TEASER_COMMENTS.length],
  ...
  const uniqueReviewers = useMemo(() => {
    // Estimate from review count (user_ids not fetched for unauthenticated visitors)
    const totalReviews = restaurants.reduce((sum, r) => sum + r.reviews.length, 0)
    return Math.max(1, Math.ceil(totalReviews / 3))
  }, [restaurants])
  ...
  const avatarInitials = useMemo(() => {
    // Generic initials for unauthenticated visitors (no real user data exposed)
    return ['TM', 'RE', 'FD', 'LB'].slice(0, Math.min(4, uniqueReviewers))
  }, [uniqueReviewers])
